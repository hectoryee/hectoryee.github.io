---
title: Docker with Conda and Jupyter
...
classes: wide
header:
  image: /assets/images/2021-08-24.png
  image_description: "A description of the image"

---

Getting pytorch docker to work with conda environment.yml and serve as jupyter.
...
```
Original Dockerfile from Pytorch.
...
```
Make changes to base conda env and run jupyter.

``` bash
sudo DOCKER_BUILDKIT=1 nvidia-docker build -t <image-name> \
--build-arg BASE_IMAGE=nvidia/cuda:10.2-cudnn8-devel-ubuntu18.04 \
--build-arg PYTHON_VERSION=3.9.1 \
--build-arg MAGMA_CUDA_VERSION=magma-cuda102 \
--build-arg TORCH_CUDA_ARCH_LIST_VAR="3.7+PTX;5.0;6.0;6.1;7.0;7.5" .
...
Run.
```
docker run -it --rm -p 8888:8888 -v "$(pwd)":/workspace --gpus all <image-name>
```
